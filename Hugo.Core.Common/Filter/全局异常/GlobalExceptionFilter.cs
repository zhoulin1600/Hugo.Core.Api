...
namespace Hugo.Core.Common.Filter
{
    ...
    /// 全局异常捕获器
    /// </summary>
    public class GlobalExceptionFilter : BaseActionFilterAsync, IAsyncExceptionFilter //IExceptionFilter
    {
        ...
        public async Task OnExceptionAsync(ExceptionContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(message) && message.Contains(errorAudit))
                message = message.Replace(errorAudit, $"（无法解析的服务，请重新编译项目）{errorAudit}");

            if (_hostEnvironment.IsDevelopment())
            { }
            
            context.Result = Error(message, StatusCodes.Status500InternalServerError);

            // MiniProfiler性能分析器
            StackExchange.Profiling.MiniProfiler.Current.CustomTiming("全局异常", message);
