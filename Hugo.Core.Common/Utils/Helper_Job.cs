...
namespace Hugo.Core.Common
{
    ...
    /// </summary>
    public static class Helper_Job
    {
        #region 私有成员
            ...
            get
            {
                if (__scheduler == null)
                {
                    lock (_lock)
                    {
                        if (__scheduler == null)
                        {
                            __scheduler = Helper_Async.RunSync(() => StdSchedulerFactory.GetDefaultScheduler());
                            Helper_Async.RunSync(() => __scheduler.Start());
                        }
                    }
                }
        ...
        public static string SetIntervalJob(Action action, TimeSpan timeSpan)
        {
            ...
            ITrigger trigger = TriggerBuilder.Create()
                ...
                .Build();
            Helper_Async.RunSync(() => _scheduler.ScheduleJob(job, trigger));
        ...
        {
            Helper_Async.RunSync(() => _scheduler.DeleteJob(new JobKey(jobId)));
